function [diff_ent, diff_ent_grad] = Const_and_Variable_Design_DifferentialEntropy_QuadHPT(mu_theta,cov_theta,dim_theta,dim_z,cov_z,N_samples_theta,N_samples_z,k,N_trials,problem,myidx,auxvariable,Nspecies,Ntime,fixed_design)
...
%   - fixed_design: if 1 it means design params are fixed in time else they
%   are variable
...
if ~exist('fixed_design','var')
     % fixed_design parameter does not exist, so default it to 0 i.e.
     % assume it is a case of variable design parameters
      fixed_design = 0;
end

% since we allow users to have either fixed or variable design, in case of
% fixed design, we need to create Ntime copies of constant design
% parameters to be used in the rest of the code below where vector of
% design parameters are expected
if fixed_design == 0
    k_vec = k;
else
    k_vec = zeros(3*Ntime - 1, 1); % [FA_P_1, FA_L_1, FA_P_2, FA_L_2, ..., FA_P_N, FA_L_N, TR_1, ..., TR_N_minus_1]
    for i=1:Ntime
        k_vec(2*(i-1)+1, 1) = k(1); % pyruvate FA
        k_vec(2*(i-1)+2, 1) = k(1); % lactate FA
        if i < Ntime
            k_vec(2*Ntime + i, 1) = k(3); % TR
        end
    end
end
...
for ii = 1:N_trials
    ...
    kt = k' % k of actual design variable (if constant, it has three elements; if variable, it has 3*Ntime - 1 elements)
    ...
    for j = 1:N_samples_theta
        % in case fixed_design = 1, the following gives derivatives with respect to
        % constant design variables (total three elements in
        % f_theta_i_grad_j in this case; two wrt pyruvate and lactate
        % and one wrt TR
        [f_theta_i_j,f_theta_i_grad_j]=Const_and_Variable_Design_MIGHQuadHPTofts(theta_i(:,j),k_vec,problem,myidx,auxvariable,Nspecies,Ntime,fixed_design); % for computing E_{theta}(.) 
        end
    ...
    for j = 1:N_samples_z
        [f_theta_i_z,~]=Const_and_Variable_Design_MIGHQuadHPTofts(theta_for_z(:,j),k_vec,problem,myidx,auxvariable,Nspecies,Ntime,fixed_design); % f_theta_i_grad_z
...
function [objfun, objfun_Der]=Const_and_Variable_Design_MIGHQuadHPTofts(theta,xopt,problem,myidx,auxvariable,Nspecies,Ntime,fixed_design)
% function [objfun,objfun_Der] =MIGHQuadHPTofts(theta,xopt,problem,myidx,auxvariable,Nspecies,Ntime)
   ...
   if nargout > 1
       ...
       if fixed_design == 0
           ...
           objfun_Der_temp = [objectiveGradFA;objectiveGradTR] +  [jacobianFA;jacobianTR    ] *   adjointvar ;
           objfun_Der = [sum(objfun_Der_temp(1:2:2*Ntime)); ...
                         sum(objfun_Der_temp(2:2:2*Ntime)); ...
                         sum(objfun_Der_temp(2*Ntime+1:end))];
       end
   end
end